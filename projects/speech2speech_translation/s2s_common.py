...
def upload_variable_to_gcs(project_id, gcs, object_name, data,
                           content_type):
...
def speech_to_text(
    project_id, location, source_language_code,
    stt_model, stt_timeout, gcs_uri_input_audio, logger):
  ...
  request = cloud_speech.BatchRecognizeRequest(
      recognizer=f"projects/{project_id}/locations/"
  ...
  logger.info("Waiting for STT operation to complete... timeout: %ss",
              stt_timeout)
...
def text_to_speech(
    project_id, location, target_voice, target_voice_gender,
    target_language_code, text, tts_timeout,
    gcs, output_audio_file_name, logger, prefix):
  ...
  gender = (
      ...
      if target_voice_gender == "female" else
  ...
  logger.info("Waiting for TTS operation to complete... timeout: %ss",
              tts_timeout)
  try:
    operation = client.synthesize_long_audio(request=request)
    result = operation.result(timeout=tts_timeout)
    logger.info("TTS output written to: %s",
                f"{gcs_path}{prefix}{output_audio_file_name}")
    if result:
