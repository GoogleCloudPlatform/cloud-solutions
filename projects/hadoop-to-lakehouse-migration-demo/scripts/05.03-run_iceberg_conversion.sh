...
# Script to run Iceberg conversion job on Managed Spark Serverless
...
if [ -z "$TARGET_PROJECT" ] || [ -z "$BRONZE_BUCKET" ] || [ -z "$SILVER_BUCKET" ] || [ -z "$METASTORE_ID" ] || [ -z "$SUBNET" ] || [ -z "$REGION" ]; then
  echo "Usage: $0 [--target-project <id>] [--bronze-bucket <name>] [--silver-bucket <name>] [--metastore-id <id>] [--scripts-dir <path>] [--subnet <name>] [--region <region>]"
  exit 1
fi

echo "Submitting Iceberg conversion job to Managed Spark Serverless..."
gcloud dataproc batches submit pyspark "$SCRIPTS_DIR/pyspark/05.03-iceberg_conversion.py" \
  --project="$TARGET_PROJECT" \
  --region="$REGION" \
  --deps-bucket="gs://$BRONZE_BUCKET" \
