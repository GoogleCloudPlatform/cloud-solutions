...
This reference architecture describes an event-driven data
processing pipeline that does not require management of underlying
infrastructure.
This document is intended for developers and cloud architects who want to
implement a serverless event processing system using Google-managed services.
...
-   [Eventarc](https://cloud.google.com/eventarc/docs) to asynchronously
    [route](https://cloud.google.com/eventarc/docs/event-providers-targets)
    events from Google Cloud sources and third-party sources about the events to
    process to targets. Eventarc
    ...
    Eventarc supports sending two event types for hundreds of Google Cloud
    sources to targets: direct events and audit events. For more information
    about choosing event types and routes, see
...
-   [Google Cloud sources and third-party sources](https://cloud.google.com/eventarc/docs/overview#sources)
    ...
    an open data format specification under the Cloud Native Computing
    Foundation.
...
-   [Prevent public access to your buckets](https://cloud.google.com/storage/docs/public-access-prevention),
    unless you need unauthenticated users to be able to access the objects
...
-   Cloud Storage can
    ...
    availability and reliability of your buckets in the unlikely event of a
    zonal or regional outage, see
...
_[sharding](https://en.wikipedia.org/wiki/Shard_(database_architecture))_. To
...
-   Amazon Simple Storage Service (Amazon S3): a scalable object storage
    service.
...
-   A deduplication to avoid processing the same event twice, while the
...
-   Benefit from having event-processing results in Google Cloud from the early
    ...
    results to get data from Firestore and Cloud Storage, without affecting the rest
    of the event-processing stack.
