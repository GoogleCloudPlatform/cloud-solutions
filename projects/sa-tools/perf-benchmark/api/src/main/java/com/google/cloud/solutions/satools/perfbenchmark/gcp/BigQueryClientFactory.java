...
/** Factory bean to instantiate Cloud BigQuery client for given credentials. */
public class BigQueryClientFactory implements ServiceFactory<BigQuery, GoogleCredentials> {

  private final UserAgentHeaderProvider headerProvider;

  public BigQueryClientFactory(UserAgentHeaderProvider headerProvider) {
  ...
  protected BigQueryOptions optionsMaker() {
    return BigQueryOptions.newBuilder().setHeaderProvider(headerProvider).build();
  }
}
