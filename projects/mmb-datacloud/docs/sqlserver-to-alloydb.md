...
flowchart TD
    subgraph VPC1 ["VPC 1 - Source Network (10.10.0.0/24)"]
        LOADER["fa:fa-server Helper VM: mssql-loader-vm<br>(BigQuery Data Preloader)"]
...
1.  Open **Cloud Shell** and verify that the lab infrastructure resources are
    deployed.
...
> [!NOTE] _The initial data import from BigQuery into SQL Server takes
...
1.  In **Cloud Shell**, create and run `deploy-tcp-proxy.sh`:
...
1.  Monitor the job status until it transitions from `Starting` to `Full dump`
    and finally to `Running` (Continuous replication phase).
