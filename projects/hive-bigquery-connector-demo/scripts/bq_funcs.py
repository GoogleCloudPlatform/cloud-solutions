...
def delete_partition_worker(queue: multiprocessing.Queue, worker_name: str,
                            bucket_name: str):
...
def export_partitioned_table(table_name: str, bucket_name: str):
  ...
  logger.info("Cleaning path gs://%s/data/%s and "
              "cleaning path gs://%s/backup/%s",
              bucket_name,
              short_table_name,
              bucket_name,
              short_table_name)
  ...
  if blobs:
    ...
    for i in range(ScriptState.processes()):
      p = multiprocessing.Process(target=delete_partition_worker, args=(
          delete_queue, f"worker-{i}", bucket_name))
      workers.append(p)
      p.start()
    for p in workers:
      p.join()
  ...
  while not all(map(lambda job: job.done(), all_jobs)):
    time.sleep(1)
  error_jobs = list(filter(lambda job: job.error_result is not None, all_jobs))
  if any(error_jobs):
  ...
  logger.info(
      "Finished extraction of table %s to partition destinations",
      table_name)
...
def extract_table(table_name: str, bucket_name: str):
  ...
  for blob in bucket.list_blobs(prefix=f"backup/{short_table_name}"):
    blob.delete()
  logger.info("Extracting of table %s to %s", table_name,
              dest_path[0])
  ...
  logger.info("Finished extraction of table %s to %s",
              table_name, dest_path[0])
  return dest_path[0]


def mount_partitioned_table(table_name: str, extracted_path: str,
  ...
  job = ScriptState.bq_client().query(query,
                                      job_id_prefix=f"mount-partition-table-"
                                                    f"{table_name}")
  while not job.done():
    time.sleep(1)
  if job.error_result is not None:
...
def copy_table(
    full_source_table_name: str,
    partitioned: typing.Optional[str] = None) -> bigquery.Table:
  """Copy a table to BigQuery."""

  source_table = ScriptState.bq_client().get_table(full_source_table_name)
  cols = [
      f"CAST({c.name} AS DATETIME) AS {c.name}" if c.field_type == "TIMESTAMP"
      else c.name
      for c in
      source_table.schema]
  target_table_short_name = source_table.table_id
  ref = bigquery.TableReference(ScriptState.dataset().reference,
                                target_table_short_name)
  partitioned = "" if partitioned is None else (f"PARTITION BY DATE("
                                                f"{partitioned})")
...
def mount_table(table_name: str, extracted_path: str, connection_location: str,
                connection_id: str):
...
def handle_order_items():
  ...
  extracted_path = extract_table(
      ...
      ScriptState.tf_state().warehouse_bucket)
...
def handle_events():
  events_table = copy_table(
      ...
      partitioned="created_at")  # type: bigquery.Table
...
def handle_distribution_center():
  ...
  extract_table("ecommerce.distribution_centers",
                ScriptState.tf_state().staging_bucket)
