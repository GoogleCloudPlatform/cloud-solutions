...
2.  Any `Event` yielded by a sub-agent (or a tool within it) sets
...
2.  **LLM-Driven Delegation (`transfer_to_agent`)**: A `LlmAgent` can
...
3.  **Explicit Invocation (`AgentTool`)**: An `LlmAgent` can treat another
...
3.  **Third-Party Tool Wrappers**: For seamless integration with other
    frameworks.
...
6.  **Dynamic Toolsets (`BaseToolset`)**: Instead of a static list of tools, use
...
3.  Agent `yield`s an `Event` (e.g., tool call, text response). Execution
    pauses.
4.  `Runner` processes the `Event` (applies state changes, etc.) and yields it
    to the client.
...
2.  **Check for Auth Response**:
    `tool_context.get_auth_response(my_auth_config)`.
3.  **Initiate Auth**: If no creds, call
...
5.  **Make API Call**: Use the valid credentials (e.g.,
...
2.  **In-Tool Guardrails**: Design tools defensively. Tools can read policies
...
4.  **Model and Tool Callbacks (LLM as a Guardrail)**: Use callbacks to inspect
    inputs/outputs.
...
6.  **Network Controls & VPC-SC**: Confine agent activity within secure
    perimeters (VPC Service Controls) to prevent data exfiltration.
7.  **Output Escaping in UIs**: Always properly escape LLM-generated content in
...
2.  **Virtual Environment (`pip` or `uv`):**
    ...
    ```
3.  **Persistent CLI Install (`pipx` or `uv tool`):** Installs globally in an
...
1.  **Create a script:** In the project's root directory, create a Python script
    named `run_agent.py`.
2.  **Invoke the agent:** In the script, write code to programmatically call
...
3.  **Run the test:** Execute your script from the terminal using `uv`.
    `bash     uv run python run_agent.py     ` You can keep the test file for
    future testing.
...
2.  **Preservation Check:** Compare your proposed `new_string` against the
