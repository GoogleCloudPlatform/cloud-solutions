...
class Interpolate(nn.Module):
    """Custom layer to imitate BilinearResize from TensorFlow.
...
class Conv2dSamePadding(nn.Module):
    """Custom Conv2D layer with TensorFlow-style 'same' padding.

    Imitates the padding="same" functionality of TensorFlow convolutions.
    """
...
class Conv2dNormActivationSamePadding(ConvNormActivation):
    """Conv2D with normalization, activation, and 'same' padding."""
...
class MBConvSamePadding(nn.Module):
    """Mobile Inverted Bottleneck Conv with 'same' padding."""
...
class PermuteLayerNHWC(nn.Module):
    """Layer to permute tensor dimensions to NHWC format."""
...
class Conv3DSamePadding(nn.Module):
    """3D convolution with TensorFlow-style 'same' padding."""
...
class MaxPool3dSame(nn.Module):
    """3D max pooling with TensorFlow-style 'same' padding."""
...
class InceptionB0B3(nn.Module):
    """Inception block B0/B3 with optional max pooling."""
    ...
    def __init__(self, conv1_in, conv1_out, pool_shape, include_maxpool=False):
        ...
        if include_maxpool:
            self.maxpool_layer = MaxPool3dSame(
...
class InceptionB1B2(nn.Module):
    """Inception block B1/B2 with spatial-temporal convolutions."""
...
class InceptionMixed(nn.Module):
    """Mixed Inception module combining multiple branch types."""
    ...
    def __init__(
        ...
        self.b1 = InceptionB1B2(
            conv1_in, b1_conv1_out, b1_conv2_out, b1_pool_shape
        )
        self.b2 = InceptionB1B2(
            conv1_in, b2_conv1_out, b2_conv2_out, b2_pool_shape
        )
...
class InceptionMixedBlock(nn.Module):
    """Full Inception block with multiple mixed inception layers."""
