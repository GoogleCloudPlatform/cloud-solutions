...
MODEL_PATH = os.path.join(
    ...
    "..",
    "checkpoint/distortionnet_pytorch_statedict.pt",
)
...
class DistortionNet(nn.Module):
    """
    DistortionNet is based on the EfficientNet architecture. One can
    achieve the same network by modifying torchvision's efficientnet_b0
    model. The features layers will be the same, but the final avgpool
    and classifier layers will need to change. Instead of
    AdaptiveAvgPool2D will need to use MaxPool, and the classifier will
    consist of two linear layers.
    In addition if the intention is to use the baseline weights from the
    tensorflow implementation, changes must be made to support the "same"
    padding used in tensorflow convolution layers.
...
class DistortionNetInference:
    """Inference helper for DistortionNet model."""
    ...
    def get_labels_and_features_for_all_frames(
        ...
        Args:
            video: 5d array of shape (num_seconds, fps, channels=3,
                height=720, width=1280) with values in [-1, 1] range

        Returns:
            Tuple containing:
                - feature: 4d array of shape (num_seconds, 16, 16,
                  channels=100) of features to be used in aggregation
                - label: 2d array of shape (num_seconds, 2, 2, 26) of
                  predicted probabilities for 26 distortion classes for each
                  frame partitioned into 2*2 patches

        Note:
            Even though the input video can have any fps, computation is
            performed only on the first frame of each second.
        """
        # TODO: allow converting video to a batch of patches and running
        # batch prediction instead of sliding window for loops
        label = np.ndarray(
            (
                video.shape[0],
                self.num_patches_y,
        ...
        for k in range(video.shape[0]):
            for j in range(self.num_patches_y):
                for i in range(self.num_patches_x):
                    ...
                    patch_feature, patch_label = self.predict_and_get_features(
                        patch
                    )
                    feature[
                        k,
