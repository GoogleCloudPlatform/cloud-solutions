...
MODEL_PATH = os.path.join(
    os.path.dirname(__file__), "..", "checkpoint/contentnet_pytorch.pt"
)
LABELS_CSV_PATH = os.path.join(
    os.path.dirname(__file__), "..", "checkpoint/contentnet_labels.csv"
)
...
class ContentNet(nn.Module):
    """
    ContentNet is based on the EfficientNet architecture. One can achieve
    the same network by modifying torchvision's efficientnet_b0 model. The
    features layers will be the same, but the final classifier layers will
    need to change. In addition if the intention is to use the baseline
    weights from the tensorflow implementation, changes must be made to
    support the "same" padding used in tensorflow convolution layers.
    In this implementation we have opted to not use the torchivision's
    efficientNet and instead implement the layers from scratch with
    introduction of a Conv2dSamePadding layer.
    """
    ...
    def __init__(self, num_classes=DIM_LABEL_CONTENT, dropout=0.2):
        ...
        stochastic_depth_prob = [
            x * stochastic_depth_prob_step for x in range(16)
        ]
...
class ContentNetInference:
    """Inference helper for ContentNet model."""

    def __init__(
        self,
        model_path=MODEL_PATH,
        num_classes=DIM_LABEL_CONTENT,
        eval_mode=True,
        pretrained=True,
    ):
    ...
    def predict_and_get_features(self, frame) -> tuple[np.ndarray, np.ndarray]:
        with torch.no_grad():
            features, label_probs = self.model(
                torch.Tensor(np.expand_dims(frame, 0))
            )
        return (
    ...
    def label_probabilities_to_text(
        ...
        Args:
            label_probs: 1d array of shape (num_classes=3862) of predicted
                probabilities for each class
            top_n: number of top predictions to return

        Returns:
            Tuple containing:
                - predicted: list of top_n predicted labels
                - probs: list of top_n predicted probabilities
                - indices: list of top_n predicted indices
        """
    ...
    def get_labels_and_features_for_all_frames(
        ...
        Args:
            video: 5d array of shape (num_seconds, fps, channels=3,
                height=496, width=496) with values in [-1, 1] range

        Returns:
            Tuple containing:
                - feature: 4d array of shape (num_seconds, 16, 16,
                  channels=100) of features to be used in aggregation
                - label: 2d array of shape (num_seconds, num_classes=3862)
                  of predicted probabilities for each class

        Note:
            Even though the input video can have any fps, computation is
