...
MODEL_PATH = os.path.join(
    ...
    "..",
    "checkpoint/compressionnet_pytorch_statedict.pt",
)
...
class CompressionNet(nn.Module):
    ...
    convolutional layers. (3) Another change would be to use the hooks to
    output the
...
class CompressionNetInference:
    """Inference helper for CompressionNet model."""
    ...
    def predict(self, patch):
        """
        patch is a 5d numpy array of shape
        (batch_size, channel, depth, width, height)
    ...
    def predict_and_get_features(self, patch) -> tuple[np.ndarray, np.ndarray]:
        """Predict and get features from a video patch.

        Args:
            patch: 5d numpy array of shape (batch_size, channel, depth=5,
                width, height). batch_size is assumed to be 1 for now.
                But should work with larger.

        Returns:
            Tuple of features and label probabilities.
        """
        with torch.no_grad():
    ...
    def get_labels_and_features_for_all_frames(
        ...
        Returns:
            Tuple containing:
                ...
                - label: 2d array of shape (num_seconds, 4, 4, 1) of
                  predicted compression level for each second*fps frame
                  partitioned into 4x4 patches
        """
