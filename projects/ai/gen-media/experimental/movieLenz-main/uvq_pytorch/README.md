...
video (in seconds). This will print out a dictionary containing UVQ scores based
on different combinations of CompressionNet, DistortionNet, and ContentNet.

Two additional arguments are accepted: `--transpose` specifies that the video
should be transposed before processing. If a `--output` argument is provided, a
file with that name will be created and the output results will be written to it
as well.
