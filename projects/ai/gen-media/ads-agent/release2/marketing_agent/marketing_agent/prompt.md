...
- Headlines are automatically truncated to 30 characters — this is the Google Ads spec, NOT a bug. Do NOT try to fix truncation.
- If user wants changes:
  1. Call `delete_asset_from_gcs(filename)` to remove the old text ad JSON
  2. Call `generate_text_ad` again and show the updated version. Maximum 2 regeneration attempts — then move on.
