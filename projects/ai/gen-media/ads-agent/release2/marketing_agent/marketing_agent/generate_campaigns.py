...
# pylint: disable=C0301, C0410, C0413, C0415, E0606, W0102, W0104, W0404, W0611, W0612, W0613, W0718, W1203, W1309, W1405, W1510, W1514
...
def _extract_xml(raw_text: str) -> str:
    ...
    if not xml_text.startswith("<campaigns"):
        log_message(
            "Generated XML doesn't start with <campaigns>. Wrapping.",
            Severity.WARNING,
        )
    ...
    if "</campaigns>" not in xml_text:
        log_message(
            "XML appears truncated — adding closing tags", Severity.WARNING
        )
...
def generate_campaigns_xml(
    ...
    ads_per_sheet = max(
        1, -(-num_image_ads // num_asset_sheets)
    )  # ceiling division
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        log_message(
            f"Generating campaigns XML (attempt {attempt}/{MAX_RETRIES})",
            Severity.INFO,
        )
        ...
        try:
            ...
            log_message(
                f"Gemini API call failed on attempt {attempt} (429={is_429}): {last_error}",
                Severity.ERROR,
            )
        ...
        if response.candidates and response.candidates[0].content:
            for part in response.candidates[0].content.parts:
                if part.text:
                    raw_text += part.text
        log_message(
            f"Generated campaigns XML (length: {len(raw_text)})", Severity.INFO
        )
        ...
        try:
            ...
            log_message(
                f"XML validation passed on attempt {attempt}", Severity.INFO
            )
            return xml_text
