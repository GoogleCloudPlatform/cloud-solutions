...
def check_asset_exists(
    ...
    if ref_type == AssetRefType.GCS:
        ...
        log_message(
            "Is Public URL. Checking status and content-type.", Severity.DEBUG
        )
        try:
            response = requests.head(reference, allow_redirects=True, timeout=5)
            if not response.ok:
                log_message(
                    "Public URL check failed. Status: {response.status_code}",
                    Severity.WARNING,
                )
            ...
            if not expected_content_types:
                log_message(
                    "Public URL check success. No expected content types specified.",
                    Severity.INFO,
                )
            ...
            for expected_type in normalized_expected_types:
                if expected_type in content_type:
                    log_message(
                        f"Public URL check success. Content-Type '{content_type}' is valid.",
                        Severity.INFO,
                    )
                    return True, ref_type

            log_message(
                f"Public URL check failed. Mismatch: Content-Type '{content_type}' is not in {normalized_expected_types}.",
                Severity.WARNING,
            )
            return False, AssetRefType.NONE
        ...
        if "/" in reference:
            try:
                ...
                log_message(
                    f"Interpreting '{reference}' as GCS path: bucket='{bucket}', path='{path}'",
                    Severity.DEBUG,
                )
                file_exists = (
        ...
        log_message(
            f"Checking as filename in artifact bucket: {GOOGLE_CLOUD_BUCKET_ARTIFACTS}",
            Severity.DEBUG,
        )
        try:
            file_exists = (
...
async def load_resource(
    ...
    if artifact:
        ...
        if image_bytes:
            log_message(
                f"Loaded artifact with artifact_name: {artifact_name}. image_bytes length: {len(image_bytes)}",
                Severity.DEBUG,
            )
    ...
    log_message(
        f"Artifact not found or empty. Attempting to load from GCS: {source_path}",
        Severity.WARNING,
    )
...
async def save_to_artifact_and_render_asset(
    ...
    *,
    ...
    if not asset.media_bytes and not asset.gcs_uri:
        raise ValueError(
            "GeneratedMedia object needs to either have `media_bytes` or `gcs_uri` set"
        )
    ...
    if not file:
        raise RuntimeError(
            f"Cannot save file to artifacts. No bytes to upload: gcs_uri: {asset.gcs_uri}. asset_name: {asset.filename}. mimetype: {asset.mime_type}"
        )
    ...
    if save_in_artifacts:
        try:
            ...
            log_message(
                f"Saved file in artifacts. URL: {asset.gcs_uri}. File size: {len(file)}. asset_name: {asset.filename}",
                Severity.INFO,
            )
        except (ValueError, RuntimeError) as e:
            log_message(
                f"Artifact service unavailable, skipping artifact save: {e}",
                Severity.WARNING,
            )
    ...
    if save_in_gcs:
        ...
        log_message(
            f"Saved file in GCS. URL: {asset.gcs_uri}. File size: {len(file)}. asset_name: {asset.filename}",
            Severity.INFO,
        )
    ...
    try:
        ...
        if add_to_temp:
            temp_artifacts = context.state.get(
                f"{context.state.TEMP_PREFIX}{TEMP_ARTIFACTS_STATE_KEY}", {}
            )
...
def get_or_create_unique_session_id(callback_context: CallbackContext):
    ...
    if not session_id:
        ...
        session_id = (
            f"{datetime.now().strftime('%Y%m%d%H%M%S%f')}_{random_suffix}"
        )
