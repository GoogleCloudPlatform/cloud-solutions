...
async def init_targeting_config(callback_context: CallbackContext):
    ...
    # Save Geotargets
    geotargets_path = os.path.join(
        current_dir, "targeting_config", "geotargets.json"
    )
    ...
    languages_path = os.path.join(
        current_dir, "targeting_config", "languagecodes.json"
    )
...
async def resolve_location_id_from_context(
    ...
    """
    logging.info(
        "TOOL: resolve_location_id_from_context called with name: %s",
        location_name,
    )
    try:
...
async def resolve_language_id_from_context(
    ...
    """
    logging.info(
        "TOOL: resolve_language_id_from_context called with name: %s",
        language_name,
    )
    try:
