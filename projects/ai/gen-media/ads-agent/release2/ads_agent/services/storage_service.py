    ...
    def get_blob(self, uri: str) -> storage.blob.Blob | None:
        ...
        try:
            ...
            if uri_clean.startswith("gs://"):
                ...
                uri_clean = uri_clean[
                    len("https://storage.cloud.google.com/") :
                ]
            ...
            logging.info(
                f"[StorageService] get_blob: bucket={bucket_name}, path={path}"
            )
            logging.info(
                f"[StorageService] get_blob: project={self.storage_client.project}"
            )
            ...
            logging.error(
                f"[StorageService] get_blob ERROR for {uri}: {type(e).__name__}: {e}"
            )
            import traceback
