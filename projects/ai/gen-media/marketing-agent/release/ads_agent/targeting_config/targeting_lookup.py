...
class TargetingLookupTool:
    ...
    _locations_file = os.path.join(
        _current_dir, "geotargets.json"
    )  # Move this to Firestore
    ...
    @staticmethod
    def search_in_dict(
        data_dict: dict, user_input: str, id_field: str, threshold: int = 75
    ):
        """
        ...
        match = process.extractOne(
            user_input, keys, scorer=fuzz.token_sort_ratio
        )
