...
def check_asset_exists(
    ...
    if ref_type == AssetRefType.GCS:
        ...
        try:
            file_exists = utils_gcs.check_if_gcs_file_exists_from_bucket_and_path(
                bucket_name=GOOGLE_CLOUD_BUCKET_ARTIFACTS, file_path=reference
            )
            log_message(
...
async def load_resource(
    ...
    if artifact:
        ...
        if image_bytes:
            ...
            return GeneratedMedia(
                media_bytes=image_bytes, mime_type=mime_type, filename=artifact_name
            )
    ...
    for gcs_uri in gcs_candidates:
        try:
            ...
            if image_bytes:
                log_message(
                    f"Successfully downloaded from GCS: {gcs_uri}", Severity.INFO
                )
                return GeneratedMedia(
                    gcs_uri=gcs_uri,
    ...
    log_message(
        f"Could not load resource from any source: {source_path}", Severity.ERROR
    )
    return None
...
def store_inline_artifact_metadata(
    ...
    try:
        ...
        log_message(
            f"Saved inline artifact metadata for: {asset.filename}", Severity.INFO
        )
    except Exception as e:
        log_message(
            f"WARNING: Failed to save inline artifact metadata: {e}", Severity.WARNING
        )
