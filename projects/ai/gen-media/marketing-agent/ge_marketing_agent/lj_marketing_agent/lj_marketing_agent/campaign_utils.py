...
class Asset(BaseModel):
    ...
    rationale: str
    id: str | None = None
    source_asset_sheet_id: str | None = None
...
class Campaign(BaseModel):
    ...
    def get_segment_by_name(self, segment_name: str) -> Segment | None:
...
def find_campaign_by_name(
    campaigns: list[Campaign], name: str
) -> Campaign | None:
...
def _get_required_text(
    element: ET.Element, tag_name: str, parent_name: str
) -> str:
...
def _parse_asset(elem: ET.Element | None, context: str) -> Asset:
    ...
    if elem.find("uri") is not None:
        ...
        if source_sheet_id_elem is not None:
            source_sheet_id = (
                source_sheet_id_elem.text.strip()
                if source_sheet_id_elem.text
                else None
            )
...
def parse_campaigns_from_xml(xml_content: str) -> list[Campaign]:
    ...
    for i, campaign_elem in enumerate(root.findall("campaign")):
        ...
        try:
            ...
            insight = _get_required_text(
                campaign_elem, "insight", context_prefix
            )
            visual_key = _get_required_text(
                campaign_elem, "visual_key", context_prefix
            )
            tagline = _get_required_text(
                campaign_elem, "tagline", context_prefix
            )
            ...
            if asset_sheets_container is not None:
                for k, asset_node in enumerate(
                    ...
                    asset_sheets.append(
                        _parse_asset(
                            asset_node,
                            f"{context_prefix} -> Asset Sheet #{k+1}",
                        )
                    )
            ...
            if not asset_sheets:
                ...
                if single_sheet_elem is not None:
                    asset_sheets.append(
                        _parse_asset(
                            single_sheet_elem,
                            f"{context_prefix} -> Single Asset Sheet",
                        )
                    )
            ...
            if not segments:
                raise ValueError(
                    f"{context_prefix}: Must define at least one segment."
                )
            ...
            raise ValueError(
                f"Validation Error in {context_prefix}: {e}"
            ) from e
