...
async def _create_display_ad_task(
    ...
    sanitized_keywords = re.sub(r"[^a-zA-Z0-9]", "_", concept_keywords).strip(
        "_"
    )
    ...
    # Construct Filename
    filename_prefix = (
        f"display_ad_{sanitized_keywords}_{timestamp_str}_{random_chars}"
    )
        ...
        try:
            ...
            if product_media and product_media.media_bytes:
                part = types.Part.from_bytes(
                    data=product_media.media_bytes,
                    mime_type=product_media.mime_type,
                )
    ...
    if reference_images_uris:
        for uri in reference_images_uris:
            generated_media: GeneratedMedia | None = (
                await utils_agents.load_resource(
            ...
            if generated_media and generated_media.media_bytes:
                ...
                reference_image_parts.append(part)
                image_descriptions.append(
                    f"REFERENCE: {generated_media.filename}"
                )

    # Construct the specialized Display Ad Prompt
    final_prompt = "CONCEPT: \n\n ```text"
    final_prompt += f"\n\n{prompt_description}"
    final_prompt += "\n\n```\n\n"
...
async def generate_display_ad(
    ...
    """Generates a final, high-quality Display Ad (image) with short copy and branding.
    ...
    try:
        ...
        if (
            ...
            best_attempt_evaluation = (
                cast(EvalResult, best_eval) if best_eval else None
            )
            ...
            return {
                "status": "failed",
                "detail": "Display Ad generation failed.",
            }
