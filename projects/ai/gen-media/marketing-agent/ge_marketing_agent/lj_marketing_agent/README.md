...
```text
marketing_agent/
├── __init__.py                    # Main agent — all tools + video pipeline
...
- ADK CLI v1.30.0 (v2.x Dockerfile-based deploy is incompatible with Agent
  Engine's runtime)

#### Important: Import Requirements

All imports across the codebase **must use relative imports** — Agent Engine
renames the package directory at deploy time, breaking absolute imports:

- Within `lj_marketing_agent/`: use `from ..adk_common.utils.xxx import yyy`
- Within `lj_marketing_agent/sub_agents/`: use
  `from ...adk_common.utils.xxx import yyy`
- Within `adk_common/utils/`: use `from .xxx import yyy`
- Within `lj_marketing_agent/` for sibling modules: use
  `from ..config import xxx`, `from ..schema import xxx`
...
- **`ModuleNotFoundError: No module named 'xxx'`** — An absolute import exists.
  Change to relative import.
- **`Invalid requirement: '/app/agents/...'`** — A Docker-style path in
  `requirements.txt`. Remove it.
- **`requires-python` mismatch** — Set `requires-python = ">=3.11"` in
  `pyproject.toml`.
...
1.  Click **Create App** > **Conversational Agent**
1.  Set app name and click **Create**
...
1.  In your GE app, go to **Agents** > **Add agent** > **Custom agent via Agent
    Runtime**
1.  Set the Reasoning Engine path:
    `projects/<PROJECT_ID>/locations/us-central1/reasoningEngines/<ENGINE_ID>`
1.  Skip Authorization (can add later)
1.  Click **Create**

#### Step 5: Add User Permissions

1.  In the GE app > **Agents** > click on your agent
1.  Go to **User permissions** > **Add user**
1.  Add the user's email with **Agent User** role

#### Step 6: Set Up Identity Provider

1.  In the GE app, go to **Integration**
1.  Select **Use Google Identity**
1.  Note the webapp URL provided
