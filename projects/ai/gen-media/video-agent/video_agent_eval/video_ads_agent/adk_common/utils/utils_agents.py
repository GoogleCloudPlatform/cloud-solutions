...
"""Agent utility helpers and asset processing routines."""
...
def check_asset_exists(
    reference: str, expected_content_types: Set[str]
) -> Tuple[bool, AssetRefType]:
    """
    ...
    Args:
        reference (str): Asset URI, URL, or filename.
        expected_content_types (List[str]): Expected mimetypes.

    Returns:
        Tuple[bool, AssetRefType]: (True if asset exists, False otherwise).
    """
    ...
    if ref_type == AssetRefType.GCS:
        ...
        log_message("Is Public URL. Checking status.", Severity.DEBUG)
        try:
            ...
            if not expected_content_types:
                log_message(
                    "Public URL check success.",
                    Severity.INFO,
                )
                return True, ref_type

            content_type = response.headers.get("content-type", "").lower()
            normalized_expected_types = [
                t.lower() for t in expected_content_types
            ]

            for expected_type in normalized_expected_types:
                if expected_type in content_type:
                    log_message(
                        f"URL check ok: '{content_type}'.",
                        Severity.INFO,
                    )
                    return True, ref_type

            log_message(
                f"URL check fail mismatch: '{content_type}'.",
                Severity.WARNING,
            )
            return False, AssetRefType.NONE
            ...
            log_message(
                f"Public URL check failed. Exception: {e}", Severity.ERROR
            )
            return False, AssetRefType.NONE
        ...
        if "/" in reference:
            try:
                # Try prepending gs:// if it doesn't have it
                test_uri = f"gs://{reference}"
                bucket, path = utils_gcs.parse_gcs_url(test_uri)
                log_message(
                    f"GCS path: bucket='{bucket}', path='{path}'",
                    Severity.DEBUG,
                )
                file_exists = (
                    utils_gcs.check_if_gcs_file_exists_from_bucket_and_path(
                        bucket_name=bucket, file_path=path
                    )
                )
                log_message(f"GCS file exists? {file_exists}", Severity.DEBUG)
                return (
                    (file_exists, AssetRefType.GCS)
                    if file_exists
                    else (False, AssetRefType.NONE)
                )
            except (
                ValueError,
                TypeError,
                ...
                # Fallback to default artifact bucket
                pass

        art_bkt = GOOGLE_CLOUD_BUCKET_ARTIFACTS
        log_message(
            f"Checking artifact bucket: {art_bkt}",
            Severity.DEBUG,
        )
        try:
            file_exists = (
            ...
            log_message(
                f"Filename check in artifact bucket exists? {file_exists}",
                Severity.DEBUG,
            )
            ...
            return (
                (file_exists, AssetRefType.FILENAME)
                if file_exists
                else (False, AssetRefType.NONE)
            )
        except (
            ...
            log_message(
                f"Filename check failed. Exception: {e}", Severity.ERROR
            )
            return False, AssetRefType.NONE
...
def download_bytes_from_reference(reference: str) -> tuple[bytes, str | None]:
    """Downloads bytes and retrieves mimetype from reference (GCS/URL/file)."""
    ref_type = classify_asset_reference(reference)
    ...
    if not asset:
        log_message(
            "Reference is empty or corrupt - could not download file bytes",
            Severity.ERROR,
        )
        raise RuntimeError(
            "Reference is empty or corrupt - could not download file bytes"
...
def transfer_reference_to_gcs(
    ...
    Args:
        ...
        destination_blob_name (str | None): The destination blob name.
                                            If None, uses reference filename.
...
def get_number_of_images(number_of_images: int, default_num) -> int:
    max_number_of_images = (
        4 if not isinstance(default_num, int) else int(default_num)
    )
...
async def load_resource(
    ...
    """
    log_message(
        f"Loading image resource from source path {source_path}", Severity.INFO
    )

    # 1. Try loading from artifact service
    # If source_path is URI, extract filename for artifact lookup
    ...
    log_message(
        f"Attempting to load as artifact: {artifact_name}", Severity.DEBUG
    )
    artifact: Optional[types.Part] = await tool_context.load_artifact(
        artifact_name
    )
    if artifact:
        ...
        if image_bytes:
            log_message(
                f"Loaded artifact {artifact_name} size: {len(image_bytes)}",
                Severity.DEBUG,
            )
    ...
    log_message(
        f"Artifact empty. Loading from GCS: {source_path}",
        Severity.WARNING,
    )
    ...
    if classify_asset_reference(source_path) == AssetRefType.GCS:
        ...
        gcs_candidates.append(
            f"gs://{GOOGLE_CLOUD_BUCKET_ARTIFACTS}/{session_id}/{source_path}"
        )
        # b) gs://{BUCKET}/{source_path}
        gcs_candidates.append(
            f"gs://{GOOGLE_CLOUD_BUCKET_ARTIFACTS}/{source_path}"
        )
        # c) gs://{source_path}
    ...
    for gcs_uri in gcs_candidates:
        try:
            log_message(
                f"Trying to download from GCS: {gcs_uri}", Severity.DEBUG
            )
            image_bytes = utils_gcs.download_bytes_from_gcs(gcs_uri)
            if image_bytes:
                log_message(
                    f"Successfully downloaded from GCS: {gcs_uri}",
                    Severity.INFO,
                )
                return GeneratedMedia(
                    gcs_uri=gcs_uri,
            ...
            log_message(
                f"Failed to download from {gcs_uri}: {e}", Severity.DEBUG
            )

    log_message(
        f"Could not load resource from any source: {source_path}",
        Severity.ERROR,
    )
    return None
...
async def save_to_artifact_and_render_asset(
    ...
    *,
    ...
    save_in_gcs: bool = True,
    save_in_artifacts: bool = False,
) -> GeneratedMedia:

    log_message(
        f"Saving asset in artifacts: {asset.filename}",
        Severity.INFO,
    )

    if not asset.media_bytes and not asset.gcs_uri:
        raise ValueError("GeneratedMedia requires `media_bytes` or `gcs_uri`")

    # Note: change this code to also allow for download from URL (not GCS)
    file = asset.media_bytes or utils_gcs.download_bytes_from_gcs(
        str(asset.gcs_uri)
    )

    if not file:
        raise RuntimeError(
            f"Cannot save file to artifacts. No bytes: {asset.filename}"
        )
    ...
    if save_in_gcs:
        bucket_path = (
            f"{GOOGLE_CLOUD_BUCKET_ARTIFACTS}/{gcs_folder}"
            if gcs_folder
            else GOOGLE_CLOUD_BUCKET_ARTIFACTS
        )
        raw_gcs_uri = utils_gcs.upload_to_gcs(
            bucket_path=bucket_path,
            destination_blob_name=asset.filename,
            file_bytes=file,
        )

        asset.gcs_uri = utils_gcs.normalize_to_authenticated_url(raw_gcs_uri)
        log_message(
            f"Saved in GCS: {asset.gcs_uri} size: {len(file)}",
            Severity.INFO,
        )

    if save_in_artifacts:
        if len(file) > 9_000_000:
            if raw_gcs_uri:
                log_message(
                    f"File too large ({len(file):,} bytes): {asset.filename}.",
                    Severity.INFO,
                )
                try:
                    await context.save_artifact(
                        filename=asset.filename,
                        artifact=types.Part.from_uri(
                            file_uri=raw_gcs_uri,
                    ...
                    log_message(
                        f"Failed to save artifact via URI: {e}",
                        Severity.WARNING,
                    )
            else:
                log_message(
                    f"Skipping inline artifact save for {asset.filename}.",
                    Severity.INFO,
                )
        else:
            try:
                ...
                log_message(
                    f"Saved inline artifact: {asset.filename}",
                    Severity.INFO,
                )
            except (ValueError, RuntimeError) as e:
                log_message(
                    f"Artifact service unavailable: {e}",
                    Severity.WARNING,
                )

    try:
        store_inline_artifact_metadata(context, asset, add_to_temp=True)
    except (
        ValueError,
        TypeError,
        KeyError,
...
def store_inline_artifact_metadata(
    context: CallbackContext, asset: GeneratedMedia, add_to_temp: bool = False
):
    ...
    try:
        ...
        if add_to_temp:
            temp_prefix = getattr(context.state, "TEMP_PREFIX", "")
            temp_artifacts = context.state.get(
                f"{temp_prefix}{TEMP_ARTIFACTS_STATE_KEY}", {}
            )
            if not isinstance(temp_artifacts, dict):
                temp_artifacts = {}
            temp_artifacts[asset.filename] = new_artifact_entry
            context.state[f"{temp_prefix}{TEMP_ARTIFACTS_STATE_KEY}"] = (
                temp_artifacts
            )

        log_message(
            f"Saved inline artifact metadata for: {asset.filename}",
            Severity.INFO,
        )
    except (
        ValueError,
        ...
        log_message(
            f"WARNING: Failed to save inline artifact metadata: {e}",
            Severity.WARNING,
        )
...
def get_or_create_unique_session_id(callback_context: CallbackContext):
    session_id = (
        get_unique_session_id(callback_context) or callback_context.session.id
    )
    if not session_id:
        random_suffix = "".join(
            random.choices(string.ascii_lowercase + string.digits, k=4)
        )
        time_prefix = datetime.now().strftime("%Y%m%d%H%M%S%f")
        session_id = f"{time_prefix}_{random_suffix}"
