...
"""Google Cloud Storage interaction utilities."""
...
def create_bucket_from_spec(
    ...
    try:
        ...
        iam_config = bucket_resource.iam_configuration
        ...
        if DEPLOYMENT_BUCKET_SOFT_DELETE_DAYS > 0:
            ...
            retention_seconds = (
                DEPLOYMENT_BUCKET_SOFT_DELETE_DAYS * 24 * 60 * 60
            )
        ...
        print(f"Creating bucket '{bucket_name}' in {bucket_location}...")
        bucket = client.create_bucket(bucket_resource)

        log_message(
            f"Bucket '{bucket_name}' created successfully with:", Severity.INFO
        )
        log_message(f"Location: {bucket.location}", Severity.INFO)
        del_sec = bucket.soft_delete_policy.retention_duration_seconds
        log_message(
            f"Soft Delete: {del_sec} seconds",
            Severity.INFO,
        )
        ...
        log_message(
            f"Bucket '{bucket_name}' already exists. Fetching.", Severity.INFO
        )
        bucket = client.get_bucket(bucket_name)

    except (ValueError, RuntimeError, OSError, GoogleAPICallError) as e:
...
def upload_to_gcs(
    ...
    Args:
        bucket_path (str): Name of GCS bucket/path.
        file_bytes (bytes): File bytes to upload.
        destination_blob_name (str): Destination blob name in bucket.
    ...
    log_message(
        f"File uploaded to gs://{bucket_name}/{destination_blob_path}",
        Severity.INFO,
    )
...
def download_bytes_from_gcs(uri: str) -> bytes | None:
    ...
    Args:
        uri (str): GCS URI of object (e.g. gs://bucket/path/file).
    ...
    try:
        ...
        log_message(
            f"File gs://{bucket_name}/{path} downloaded as bytes.",
            Severity.INFO,
        )
        return file_bytes
...
def parse_gcs_url(uri: str) -> tuple[str, str]:
    ...
    Args:
        uri: A string representing the GCS URI (e.g.,
...
def check_if_gcs_file_exists_from_string(
    ...
    try:
        ...
        log_message(
            f"Is GCS URI. Bucket: '{bucket}', Path: '{path}'", Severity.DEBUG
        )
        file_exists = check_if_gcs_file_exists_from_bucket_and_path(
            bucket, path
        )
        log_message(f"GCS file exists? {file_exists}", Severity.DEBUG)
    except ValueError:
        # It's not a GCS URI, so we return False.
        log_message("Not a GCS URI, proceeding...", Severity.DEBUG)
    except (
        TypeError,
        KeyError,
        AttributeError,
        ...
        log_message(
            f"GCS parsing failed unexpectedly. Exception: {e}",
            Severity.WARNING,
        )

    if not file_exists and optional_bucket:
        return check_if_gcs_file_exists_from_bucket_and_path(
            optional_bucket, reference
        )
...
def normalize_to_gs_bucket_uri(path: str) -> str:
    ...
    Returns:
        The normalized GCS object URI in the format
        'gs://<bucket-name>/<path-to-file>'.
    """
    if path.startswith("gs://"):
        # It's already in the correct format, but we should unquote the path
        # to handle any percent-encoded characters from sources like signed
        # URLs.
        parsed = urlparse(path)
        return urlunparse(
            ("gs", parsed.netloc, unquote(parsed.path), "", "", "")
        )
        ...
        if len(path_parts) < 2:
            # Handle cases like "my-bucket" which should be converted to
            # "gs://my-bucket"
...
def normalize_to_authenticated_url(path: str) -> str:
    ...
    2. A public GCS HTTP URL (e.g.,
    'https://storage.googleapis.com/my-bucket/folder/file.txt')
    3. A path that includes the bucket and object (e.g.,
    ...
    for blob in blobs:
        ...
        if blob.name.endswith(".txt") or blob.name.endswith(".md"):
            ...
            try:
                ...
                log_message(
                    f"    Error processing {blob.name}: {e}", Severity.ERROR
                )
            ...
            log_message(
                f"  - Skipping (not a .txt or .md file): {blob.name}",
                Severity.DEBUG,
            )
...
def download_text_from_gcs(uri: str) -> str:
    """Downloads a file from Google Cloud Storage and returns its content as
    bytes.

    Args:
        uri (str): The GCS URI of the object (e.g.,
        "gs://bucket_name/path/to/file").
...
def get_files_metadata_from_gcs_bucket(
    ...
    try:
        ...
        log_message(
            f"Error initializing GCS client or bucket: {e}", Severity.ERROR
        )
        return []
    ...
    log_message(
        f"Listing files in gs://{bucket_name}/{prefix_str}...", Severity.INFO
    )
    try:
        blobs = storage_client.list_blobs(bucket_name, prefix=prefix)
...
def generate_min_image(source_blob_name: str):
    """Generates a thumbnail (minified) version of a GCS image if it doesn't
    exist."""
    storage_client = storage.Client()
