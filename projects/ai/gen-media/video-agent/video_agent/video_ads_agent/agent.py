    ...
    prompts = (
        ...
        if prompt_override
            ...
            (
                ...
                "Slow Ken Burns camera within the image frame. Natural motion only. "
                "Show ONLY what is in the reference image — add nothing. Silent. 16:9."
            ),
        ]
    )
    ...
    for attempt in range(max_attempts):
        ...
        try:
                ...
                if stype == "model_output":
                    ...
                    for content in scontents:
                        ...
                        if data:
                            ...
                            print(
                                f"[Omni] Scene {scene_number}: success, {len(data):,} bytes (audio stripped)"
                            )
                            return data
        except Exception as e:
            err_str = str(e)
            ...
            error_type = (
                "CONTENT_FILTER"
                if is_content_block
                else ("RATE_LIMIT" if is_429 else "ERROR")
            )
            print(
                f"[Omni] Scene {scene_number} attempt {attempt + 1}/{max_attempts} {error_type}: {e}"
            )
            if is_content_block and attempt < max_attempts - 1:
                print(
                    f"[Omni] Scene {scene_number}: retrying with simplified prompt..."
                )
            ...
            if is_429 and attempt < max_attempts - 1:
                ...
                print(
                    f"[Omni] Scene {scene_number}: rate limited, waiting {backoff:.0f}s..."
                )
...
async def generate_scene_video_veo(
    ...
    try:
        ...
        if abs(current_ratio - target_ratio) > 0.05:
            ...
            print(
                f"[Veo] Scene {scene_number}: cropped to 16:9 ({img.size[0]}x{img.size[1]})"
            )
        buf = io.BytesIO()
        img.save(buf, format="JPEG", quality=95)
        image_bytes = buf.getvalue()
        print(
            f"[Veo] Scene {scene_number}: image ready as JPEG ({len(image_bytes):,} bytes)"
        )
    except ImportError:
        print(
            f"[Veo] Scene {scene_number}: Pillow not available, sending image as-is"
        )
    except Exception as e:
    ...
    for attempt in range(3):
        try:
            print(
                f"[Veo] Scene {scene_number}: submitting {clip_duration}s clip (attempt {attempt + 1}/3)..."
            )
            ...
            if not operation.done:
                print(
                    f"[Veo] Scene {scene_number}: timed out after {max_polls * 10}s"
                )
                continue

            if operation.error:
                print(
                    f"[Veo] Scene {scene_number}: operation error: {operation.error}"
                )
                continue
            ...
            print(
                f"[Veo] Scene {scene_number}: success, {len(generated.video.video_bytes):,} bytes"
            )
            return generated.video.video_bytes

        except Exception as e:
            print(
                f"[Veo] Scene {scene_number} attempt {attempt + 1}/3 failed: {e}"
            )
...
async def check_scene_video_veo(operation_name: str) -> bytes | str | None:
    ...
    try:
        operation = client.operations.get(types.Operation(name=operation_name))
    except Exception as e:
...
def _build_tts_prompt(script: str, emotion: str = "Energetic") -> str:
    ...
    style = style_map.get(
        emotion_lower,
        f"Speak with a {emotion} tone — match this requested emotion as accurately as possible.",
    )
    ...
    for attempt in range(3):
        try:
            ...
            if response.audio_content:
                print(
                    f"[Gemini TTS] Voiceover generated ({emotion}): {len(response.audio_content)} bytes"
                )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if result.returncode == 0:
            with open(out_path, "rb") as f:
                mixed = f.read()
                print(
                    f"[MixAudio] Success: {len(mixed):,} bytes (original: {len(video_bytes):,})"
                )
                return mixed
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if tagline:
            ...
            try:
                ...
                try:
                    font = ImageFont.truetype(_find_font(), 80)
                except Exception:
                ...
                filter_complex += (
                    f"{curr_v}[{tagline_idx}:v]overlay=(W-w)/2:H-h-100[v2];"
                )
                curr_v = "[v2]"
            except Exception as e:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if tagline:
            ...
            try:
                ...
                try:
                    font = ImageFont.truetype(_find_font(), 54)
                except Exception:
                    font = ImageFont.load_default()
        ...
        enable_expr = (
            f":enable='lt(t,{duration - 4.0})'" if duration > 4.5 else ""
        )
        ...
        if logo_idx > 0 and tagline_idx > 0:
            ...
            filter_complex += f"[0:v][{tagline_idx}:v]overlay=(W-w)/2:H-h-40{enable_expr}[out]"
        else:
            return video_bytes
...
def concatenate_scenes_with_dissolve(
    ...
    """Concatenate video clips with dissolve (crossfade) transitions using ffmpeg xfade.
    Uses an iterative approach (combining 2 clips at a time) to prevent FFmpeg OOM crashes
    caused by chaining multiple xfade filters in a single filtergraph.
    """
    if not clip_bytes_list:
        return None
    if len(clip_bytes_list) == 1:
    ...
    def xfade_two_clips(
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            c1_dur = (
                float(probe.stdout.strip()) if probe.returncode == 0 else 8.0
            )
            ...
            c2_dur = (
                float(probe2.stdout.strip()) if probe2.returncode == 0 else 8.0
            )

            if c1_dur < 1.0 or c2_dur < 1.0:
                filter_complex = (
                    "[0:v]scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,format=yuv420p,setsar=1,fps=30[v0];"
                    "[1:v]scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,format=yuv420p,setsar=1,fps=30[v1];"
            ...
            v_out_path = os.path.join(td, "v_out.mp4")
            a_out_path = os.path.join(td, "a_out.m4a")

            v_filter = (
                f"[0:v]scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,format=yuv420p,setsar=1,fps=30[v0];"
                f"[1:v]scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,format=yuv420p,setsar=1,fps=30[v1];"
                f"[v0][v1]xfade=transition=fade:duration={dissolve_duration}:offset={offset:.3f}[vout]"
            )
            ...
            try:
                res_v = subprocess.run(v_cmd, capture_output=True, timeout=60)
                if res_v.returncode != 0:
                    raise Exception(
                        f"Video xfade failed (code {res_v.returncode}): {res_v.stderr.decode(errors='replace')[-500:]}"
                    )

                res_a = subprocess.run(a_cmd, capture_output=True, timeout=60)
                if res_a.returncode != 0:
                    raise Exception(
                        f"Audio acrossfade failed (code {res_a.returncode}): {res_a.stderr.decode(errors='replace')[-500:]}"
                    )

                res_mix = subprocess.run(
                    mix_cmd, capture_output=True, timeout=60
                )
                if res_mix.returncode != 0:
                    raise Exception(
                        f"Mux failed (code {res_mix.returncode}): {res_mix.stderr.decode(errors='replace')[-500:]}"
                    )

                return True
            except Exception as e:
                print(f"[Dissolve] Iterative xfade failed: {e!s}", flush=True)
                filter_complex = (
                    "[0:v]scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,format=yuv420p,setsar=1,fps=30[v0];"
                    "[1:v]scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,format=yuv420p,setsar=1,fps=30[v1];"
                    "[0:a]aformat=sample_rates=48000:channel_layouts=stereo[a0];"
                ...
                if res.returncode != 0:
                    print(
                        f"[Dissolve] Fallback concat failed (code {res.returncode}): {res.stderr.decode(errors='replace')[-500:]}",
                        flush=True,
                    )
    ...
    for mp3_path in [
        ...
        try:
            ...
            if mp3_bytes:
                ...
                if mp4_bytes:
                    print(
                        f"Voice preview {voice_name}: wrapped MP3→MP4 on-the-fly ({len(mp4_bytes)//1024} KB)"
                    )
                    return mp4_bytes
        except Exception as e:
            print(
                f"Voice preview {voice_name}: MP3 load failed from {mp3_path}: {e}"
            )
...
async def preview_voice(
    ...
    if voice_name.strip().isdigit():
        ...
        if gender not in ("male", "female"):
            return {
                "status": "error",
                "details": "Provide gender ('male' or 'female') when using a number.",
            }
        voices = GEMINI_TTS_VOICES[gender]
        if num < 1 or num > len(voices):
            return {
                "status": "error",
                "details": f"Number {num} out of range. {gender} has {len(voices)} voices (1-{len(voices)}).",
            }
        ...
        if voice_name not in all_voices:
            return {
                "status": "error",
                "details": f"Unknown voice '{voice_name}'. Use list_voices to see available voices.",
            }
    ...
    if not mp4_bytes:
        return {
            "status": "error",
            "details": f"Voice preview failed for {voice_name} with emotion '{emotion}'.",
        }
...
async def load_images_from_bucket(
    ...
    if not image_blobs:
        return {
            "status": "error",
            "details": f"No scene image files found in {bucket_uri}. Supported formats: {', '.join(IMAGE_EXTENSIONS)}",
        }
...
async def confirm_images(
    ...
    if not candidates:
        return {
            "status": "error",
            "details": "No image candidates found. Call load_images_from_bucket first.",
        }
    ...
    return {
        ...
        "next_step": "All scene images assigned. Now generate voiceover scripts — say **`'ai'`** for AI-generated or provide your own as 1- ..., 2- ..., etc.",
    }
    ...
    if not logo_bytes and target_uri:
        if (
            ...
            try:
                ...
                print(
                    f"[Store Logo] Download from reference {target_uri} failed: {e}"
                )
...
async def setup_video_ad(
    ...
    """
    print(
        f"[Tool] setup_video_ad called: company={company_name}, scenes={num_scenes}, model={video_model}",
        flush=True,
    )
...
async def store_scene_script(
    ...
    return {
        ...
        "next_step": (
            "All scripts stored. Ready to generate clips with generate_all_clips."
            if stored_count == num_scenes
...
async def generate_ai_scripts(tool_context: ToolContext) -> dict:
    ...
    if missing:
        return {
            "status": "error",
            "details": f"Missing images for scenes: {missing}. Use store_scene_image first.",
        }
...
async def generate_scene_clip(
    ...
    if not image_uri:
        return {
            "status": "error",
            "details": f"No image stored for scene {scene_number}. Use store_scene_image first.",
        }
    if not script:
        return {
            "status": "error",
            "details": f"No script stored for scene {scene_number}. Use store_scene_script or generate_ai_scripts first.",
        }
    ...
    print(
        f"[Tool] generate_scene_clip called: scene={scene_number}, model={video_model}",
        flush=True,
    )
...
async def generate_all_clips(tool_context: ToolContext) -> dict:
    ...
    print(
        f"[GenerateClips] >>>>>> TOOL ENTRY at {_time.strftime('%H:%M:%S')} <<<<<<",
        flush=True,
    )
    ...
    print(
        f"[GenerateClips] Config: model={video_model}, scenes={num_scenes}, session={session_folder}",
        flush=True,
    )
    ...
    if missing_scripts:
        print(
            f"[GenerateClips] FAIL: missing scripts for scenes {missing_scripts}",
            flush=True,
        )
        return {
            "status": "error",
    ...
    print(
        "[GenerateClips] Starting TTS + Lyria + clips ALL in parallel (TTS/Lyria in BG)...",
        flush=True,
    )
    asyncio.create_task(_bg_tts())
    asyncio.create_task(_bg_lyria())
    ...
    print(
        f"[GenerateClips] Clip generation for {len(scenes_data)} scenes with '{video_model}' (elapsed: {_time.time()-_tool_start:.1f}s)...",
        flush=True,
    )
    MAX_CONCURRENT = 4
    ...
    async def _gen_one(s):
        async with sem:
            sn = s["scene_number"]
            print(
                f"[GenerateClips Scene {sn}] Fetching image from {s['image_uri']}..."
            )
            try:
                image_bytes = utils_gcs.download_bytes_from_gcs(s["image_uri"])
                print(
                    f"[GenerateClips Scene {sn}] Image fetched: {len(image_bytes):,} bytes"
                )
            except Exception as e:
                print(
                    f"[GenerateClips Scene {sn} ERROR] Image download failed: {e}"
                )
                return {
                    "scene_number": sn,
                    "status": "failed",
            ...
            print(
                f"[GenerateClips Scene {sn}] Calling video model '{video_model}'...",
                flush=True,
            )
            initialize_ffmpeg_if_needed()
            ...
            if video_model == "veo":
                ...
                if lro_name:
                    ...
                    print(
                        f"[GenerateClips Scene {sn}] Veo LRO stored: {lro_name}",
                        flush=True,
                    )
            ...
            if not clip_res:
                print(
                    f"[GenerateClips Scene {sn} ERROR] Model video generation returned None",
                    flush=True,
                )
                return {
                    "scene_number": sn,
            ...
            print(
                f"[GenerateClips Scene {sn}] Video generated! Size: {clip_size:,} bytes. Uploading to GCS...",
                flush=True,
            )
    ...
    print(
        f"[GenerateClips] GC collected. Clip generation took {_clips_elapsed:.1f}s total",
        flush=True,
    )
    ...
    for r in results:
        if isinstance(r, Exception):
            print(
                f"[GenerateClips ERROR] Task exception: {type(r).__name__}: {r}",
                flush=True,
            )
    ...
    print(
        f"[GenerateClips COMPLETE] {succeeded}/{num_scenes} clips succeeded/submitted in {_clips_elapsed:.1f}s",
        flush=True,
    )
    ...
    if succeeded == 0:
        ...
        tts_task.cancel()
        if "lyria_task" in locals() and lyria_task:
            lyria_task.cancel()
        return {
            "status": "error",
    ...
    print(
        f"[GenerateClips] >>>>>> RETURNING ({succeeded}/{num_scenes} clips, BG audio running, {_total_elapsed:.1f}s) <<<<<<",
        flush=True,
    )
...
async def regenerate_scene_clip(
    ...
    """
    print(
        f"[Tool] regenerate_scene_clip called, scene={scene_number}", flush=True
    )
...
async def switch_model_and_regenerate(
    ...
    print(
        f"[Model Switch] Switching video model from {current_model} to {new_model}"
    )
...
async def prepare_assembly(tool_context: ToolContext) -> dict:
    ...
    try:
        ...
        print(
            f"[PrepAssembly] Config: scenes={num_scenes}, voice={voice_name}, session={session_folder}",
            flush=True,
        )
        ...
        tool_context.state["va_tagline"] = tagline
        print(
            f"[PrepAssembly] Tagline resolved: '{tagline}' ({_time.time() - t0:.1f}s)",
            flush=True,
        )
        ...
        for idx, sn in enumerate(ordered_scenes):
            ...
            print(
                f"[PrepAssembly] === Scene {sn} ({idx+1}/{len(ordered_scenes)}) ===",
                flush=True,
            )
            ...
            if not clip_bytes:
                print(
                    f"[PrepAssembly] Scene {sn}: FAIL - clip download returned None",
                    flush=True,
                )
                continue
            print(
                f"[PrepAssembly] Scene {sn}: clip downloaded ({len(clip_bytes):,} bytes, {_time.time() - t0:.1f}s)",
                flush=True,
            )
            ...
            if vo_uri:
                print(
                    f"[PrepAssembly] Scene {sn}: downloading pre-computed TTS from {vo_uri}...",
                    flush=True,
                )
                vo_bytes = utils_gcs.download_bytes_from_gcs(vo_uri)
                if vo_bytes:
                    print(
                        f"[PrepAssembly] Scene {sn}: TTS from GCS ({len(vo_bytes):,} bytes, {_time.time() - t0:.1f}s)",
                        flush=True,
                    )
            if not vo_bytes and script:
                ...
                print(
                    f"[PrepAssembly] Scene {sn}: generating TTS with voice '{voice_name}' and emotion '{emotion}'...",
                    flush=True,
                )
                ...
                if vo_bytes:
                    print(
                        f"[PrepAssembly] Scene {sn}: TTS generated ({len(vo_bytes):,} bytes, {_time.time() - t0:.1f}s)",
                        flush=True,
                    )
            if not vo_bytes:
                print(
                    f"[PrepAssembly] Scene {sn}: no TTS available ({_time.time() - t0:.1f}s)",
                    flush=True,
                )
            ...
            if vo_bytes:
                t0 = _time.time()
                print(
                    f"[PrepAssembly] Scene {sn}: trimming clip to voiceover duration...",
                    flush=True,
                )
                clip_bytes = trim_clip_to_voiceover(
                    clip_bytes, vo_bytes, pad_before, pad_after
                )
                print(
                    f"[PrepAssembly] Scene {sn}: trimmed ({len(clip_bytes):,} bytes, {_time.time() - t0:.1f}s)",
                    flush=True,
                )

            t0 = _time.time()
            print(
                f"[PrepAssembly] Scene {sn}: mixing voiceover audio...",
                flush=True,
            )
            ...
            print(
                f"[PrepAssembly] Scene {sn}: audio mixed ({len(clip_bytes):,} bytes, {_time.time() - t0:.1f}s)",
                flush=True,
            )
            if vo_bytes:
                del vo_bytes
            ...
            print(
                f"[PrepAssembly] Scene {sn}: uploading processed clip as {processed_name}...",
                flush=True,
            )
            ...
            print(
                f"[PrepAssembly] Scene {sn}: uploaded to {processed_uri} ({_time.time() - t0:.1f}s)",
                flush=True,
            )
            print(
                f"[PrepAssembly] Scene {sn}: COMPLETE in {_time.time() - scene_start:.1f}s",
                flush=True,
            )
        ...
        print(
            f"[PrepAssembly] ALL SCENES PREPARED in {total:.1f}s. Ready for assemble_final_video.",
            flush=True,
        )
        ...
        print(
            f"[PrepAssembly FATAL] {type(e).__name__}: {e} (after {elapsed:.1f}s)",
            flush=True,
        )
...
async def assemble_final_video(tool_context: ToolContext) -> dict:
    ...
    if not tool_context.state.get("va_assembly_prepared"):
        ...
        if prep_result.get("status") != "prepared":
            print(
                f"[Assembly] prepare_assembly failed: {prep_result}", flush=True
            )
            return prep_result
    ...
    try:
        ...
        print(
            f"[Assembly] Config: scenes={ordered_scenes}, music={enable_music}, session={session_folder}",
            flush=True,
        )
        ...
        t0 = _time.time()
        print(
            f"[Assembly 1/6] Downloading {len(ordered_scenes)} processed clips from GCS...",
            flush=True,
        )
        assembled_clips = []
        total_clip_bytes = 0
        for sn in ordered_scenes:
            ...
            if not uri:
                print(
                    f"[Assembly 1/6] Scene {sn}: NO URI found in state, skipping",
                    flush=True,
                )
                continue
            print(
                f"[Assembly 1/6] Scene {sn}: downloading {source} clip from {uri}...",
                flush=True,
            )
        ...
        gc.collect()
        print(
            f"[Assembly 1/6] DONE: {len(assembled_clips)} clips, {total_clip_bytes:,} total bytes ({_time.time() - t0:.1f}s)",
            flush=True,
        )
        ...
        if logo_uri:
            try:
                ...
                if logo_bytes:
                    print(
                        "[Assembly 2/6] Logo downloaded for outro. Removing background...",
                        flush=True,
                    )
        ...
        t0 = _time.time()
        print(
            f"[Assembly 3/6] Concatenating {len(assembled_clips)} clips with dissolve transitions...",
            flush=True,
        )
        ...
        print(
            f"[Assembly 3/6] DONE: {len(final_video):,} bytes ({_time.time() - t0:.1f}s)",
            flush=True,
        )
        ...
        if enable_music:
            ...
            if music_uri:
                print(
                    f"[Assembly 4/6] Downloading pre-computed Lyria music from {music_uri}...",
                    flush=True,
                )
                music_bytes = utils_gcs.download_bytes_from_gcs(music_uri)
                if music_bytes:
                    print(
                        f"[Assembly 4/6] Music from GCS ({len(music_bytes):,} bytes, {_time.time() - t0:.1f}s)",
                        flush=True,
                    )
            if not music_bytes:
                print(
                    "[Assembly 4/6] Generating Lyria background music (no pre-computed)...",
                    flush=True,
                )
                try:
                    music_bytes = await generate_background_music(
        ...
        if logo_uri or tagline:
            ...
            if logo_uri:
                try:
                    ...
                    if logo_bytes:
                        print(
                            f"[Assembly 5/6] Logo downloaded ({len(logo_bytes):,} bytes). Removing background...",
                            flush=True,
                        )
                        logo_bytes = remove_logo_background(logo_bytes)
                except Exception as e:
        ...
        print(
            f"[Assembly 6/6] Uploaded to {final_uri} ({_time.time() - t0:.1f}s)",
            flush=True,
        )

        print(
            f"[Assembly] Creating GeneratedMedia artifact (media_bytes={final_size:,}B)...",
            flush=True,
        )
        ...
        print(
            "[Assembly] Calling save_to_artifact_and_render_asset (save_in_gcs=True)...",
            flush=True,
        )
        ...
        print(
            f"[Assembly] Artifact saved and rendered ({_time.time()-t_artifact:.1f}s)",
            flush=True,
        )
        del final_video
        gc.collect()
        ...
        print(
            f"[Assembly] >>>>>> ASSEMBLY COMPLETE — total {elapsed:.1f}s, final video {final_size:,} bytes <<<<<<",
            flush=True,
        )
        current_model = tool_context.state.get("va_video_model", "omni")
...
async def add_music_to_video(tool_context: ToolContext) -> dict:
    ...
    try:
        ...
        if not final_uri:
            return {
                "status": "error",
                "details": "No final video found. Run assemble_final_video first.",
            }
        ...
        if not music_bytes:
            return {
                "status": "warning",
                "details": "Music generation failed. Video remains without background music.",
            }
        ...
        print(
            f"[AddMusic] Uploading updated video ({len(final_video):,} bytes)...",
            flush=True,
        )
        final_uri = utils_gcs.upload_to_gcs(
            bucket_path=f"{GOOGLE_CLOUD_BUCKET_ARTIFACTS}/{session_folder}",
...
def _strip_large_inline_data(callback_context, llm_request) -> None:
    ...
    try:
        ...
        print(
            f"[BeforeModel] {total} contents, stripped_inline={stripped_inline}, truncated={truncated_text}",
            flush=True,
        )
    except Exception as e:
...
async def check_veo_status(
    tool_context: ToolContext, check_now: bool = True
) -> dict:
    ...
    for sn in range(1, num_scenes + 1):
        ...
        if isinstance(clip_res, bytes):
            clip_bytes = clip_res
            print(
                f"[CheckVeo Scene {sn}] Video generated! Size: {len(clip_bytes):,} bytes. Uploading to GCS..."
            )
            clip_filename = f"scene_{sn}_clip.mp4"
            clip_uri = utils_gcs.upload_to_gcs(
                bucket_path=f"{GOOGLE_CLOUD_BUCKET_ARTIFACTS}/{session_folder}",
    ...
    if not any_found:
        ...
        if has_clips:
            return {
                ...
                "next_step": "Now call assemble_final_video to create the final video ad.",
            }
        return {
            "status": "not_started",
            "message": "No active Veo operations found and no clips stored. Generate clips first with generate_all_clips.",
        }
    ...
    if all_done and not any_failed:
        ...
        links = "\n".join(
            [
                f"- Scene {sn}: [View Clip]({tool_context.state.get(f'va_scene_{sn}_clip_uri')})"
                for sn in completed
        ...
        if completed:
            links = "\n".join(
                [
                    f"- Scene {sn}: {tool_context.state.get(f'va_scene_{sn}_clip_uri')}"
                    for sn in completed
                ]
            )
            return {
