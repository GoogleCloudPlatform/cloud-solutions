...
def build_omni_prompt(
    ...
    """Build default Omni prompt. Exposed so UI can show/edit it."""
    ...
    return (
        ...
        "- Focus ONLY on content visible in reference image\n"
        "- Do NOT add, duplicate, or modify ANY details\n"
        "- Do NOT add new objects, people, animals, or elements\n"
        "- Do NOT remove, move, or change anything from image\n"
        "- Match reference image exactly in number and position\n\n"
        "CAMERA — CONTAINED WITHIN THE IMAGE:\n"
        "- Slow camera movement: gentle zoom-in, slow push\n"
        "- NEVER move camera beyond edges of reference image\n"
        "- Do NOT reveal new areas — stay within what image shows\n"
...
def build_veo_prompt(
    ...
    """Build default Veo prompt. Exposed so UI can show/edit it."""
    brand_line = f" for {company_name}" if company_name else ""
    context_block = (
        f"\nBrand context: {brand_context}\n" if brand_context else ""
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if logo_idx > 0 and tagline_idx > 0:
            ...
            filter_complex += (
                ...
                f"(W-w)/2:H-h-40{enable_expr}[out]"
            )
        else:
            return video_bytes
...
def probe_video_resolution(video_bytes: bytes) -> tuple[int, int]:
    """
    Probe a video's resolution. Returns (width, height), defaults to (1920,
    1080)."""
    with tempfile.TemporaryDirectory() as tmpdir:
    ...
    def xfade_two_clips(
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            v_filter = (
                ...
                f"duration={dissolve_duration}:offset={offset:.3f}[vout]"
            )
            ...
            try:
                res_v = subprocess.run(
                    v_cmd, capture_output=True, timeout=60, check=False
                )
                if res_v.returncode != 0:
                    err_msg = res_v.stderr.decode(errors="replace")[-500:]
                    raise RuntimeError(
                        f"Video xfade failed (code {res_v.returncode}):"
                        f" {err_msg}"
                    )

                res_a = subprocess.run(
                    a_cmd, capture_output=True, timeout=60, check=False
                )
                if res_a.returncode != 0:
                    err_msg = res_a.stderr.decode(errors="replace")[-500:]
                    raise RuntimeError(
                        f"Audio acrossfade failed (code {res_a.returncode}):"
                        f" {err_msg}"
                    )

                res_mix = subprocess.run(
                    mix_cmd, capture_output=True, timeout=60, check=False
                )
                if res_mix.returncode != 0:
                    err_msg = res_mix.stderr.decode(errors="replace")[-500:]
                    raise RuntimeError(
                        f"Mux failed (code {res_mix.returncode}): {err_msg}"
                    )
                ...
                print(
                    f"[Dissolve] Iterative xfade failed: {str(e)}", flush=True
                )
                filter_complex = (
                    "[0:v]scale=1920:1080:"
                    "force_original_aspect_ratio=decrease,"
                ...
                if res.returncode != 0:
                    err_msg = res.stderr.decode(errors="replace")[-500:]
                    print(
                        f"[Dissolve] Fallback concat failed (code"
                        f" {res.returncode}): {err_msg}",
                        flush=True,
                    )
...
def add_background_music_to_final(
    ...
    """
    Layer background music at specified volume across the full final
    video."""
    with tempfile.TemporaryDirectory() as tmpdir:
        vid_path = os.path.join(tmpdir, "video.mp4")
        music_path = os.path.join(tmpdir, "music.wav")
    ...
    try:
        ...
        for idx, sn in enumerate(ordered_scenes):
            ...
            if vo_bytes:
                t0 = _time.time()
                print(
                    f"[PrepAssembly] Scene {sn}: trimming"
                    f" clip to voiceover duration...",
                    flush=True,
                )
                ...
                print(
                    f"[PrepAssembly] Scene {sn}: trimmed"
                    f" ({len(clip_bytes):,} bytes, {_time.time() - t0:.1f}s)",
                    flush=True,
                )
...
def _strip_large_inline_data(_callback_context, llm_request) -> None:
    ...
    try:
        ...
        for idx, content in enumerate(llm_request.contents):
            ...
            def make_text_part(txt):
                try:
                    if Part is not None and isinstance(part, Part):
