...
"""Performs QA checks on generated media using a generative model."""
...
# Cache semaphores per event loop to avoid bound errors
...
def _get_internal_prompt(
    ...
    if mime_type in ("image/png", "image/jpeg"):
        ...
        Evaluate the following media based on this criterion:
        '{evaluation_criteria}'.

        Your response must be in JSON.
        - If the media passes, respond with: {{"decision": "Pass"}}
        - If it fails, respond with: {{"decision": "Fail", "reason": "Reason"}}
        """
...
async def evaluate_media(
    ...
    try:
        ...
        log_message(
            f"Evaluating media STARTING. Time: {now_str}",
            Severity.INFO,
        )
        async with get_evaluation_semaphore():
            ...
            if not result:
                message = f"Evaluation failed. Unparsed response: {response}"
                log_message(message, Severity.ERROR)
                return EvalResult(
                    decision="Fail",
                    reason=message,
                    improvement_prompt="Evaluation failed due to parse error.",
            ...
            if result.decision.lower() == "fail":
                log_message(
                    f"Evaluation failed reason: {result.reason}",
                    Severity.WARNING,
                )
            else:
                log_message(
                    f"Overall succeeded reason: {result.reason}", Severity.INFO
                )
        ...
        end_str = datetime.datetime.now().strftime("%H:%M:%S.%f")
        log_message(
            f"Evaluating media ENDING. Time: {end_str}",
            Severity.INFO,
        )
