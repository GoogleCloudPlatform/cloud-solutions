...
async def save_to_artifact_and_render_asset(
    ...
    if save_in_artifacts:
        ...
        # Compress images to a lightweight JPEG preview to ensure fast inline rendering
        # while preventing history payload bloat.
        if mime_type and mime_type.startswith("image/"):
            try:
                ...
                mime_type = "image/jpeg"
                log_message(f"Compressed image preview for artifact {asset.filename}: {len(file):,}B -> {len(artifact_bytes):,}B", Severity.INFO)
            except Exception as e:
                log_message(f"Image preview compression failed: {e}", Severity.WARNING)

        # Compress videos > 1MB (like scene clips) to low-res silent MP4 previews to ensure fast inline rendering
        elif mime_type and mime_type.startswith("video/") and len(file) > 1_000_000:
            try:
                import tempfile
                import os
                import subprocess
                ...
                with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as inf, \
                    ...
                    # For the final video, keep a highly compressed mono audio track instead of stripping it
                    # Use higher resolution (720p) and lower CRF (26) to keep text and logo overlays crisp.
                    is_final_video = asset.filename and "final_video" in asset.filename

                    if is_final_video:
                        audio_opts = ["-c:a", "aac", "-ac", "1", "-b:a", "32k"]
                        scale_opt = "scale=-2:360"
                        crf_opt = "30"
                    else:
                        audio_opts = ["-an"]
                        scale_opt = "scale=-2:360"
                    ...
                    log_message(f"Compressed video preview for artifact {asset.filename}: {len(file):,}B -> {len(artifact_bytes):,}B", Severity.INFO)
            except Exception as e:
                log_message(f"Video preview compression failed: {e}", Severity.WARNING)
            finally:
                try:
                    os.unlink(inf.name)
                    os.unlink(outf.name)
        ...
        if len(artifact_bytes) > 5_000_000 and raw_gcs_uri:
            if raw_gcs_uri:
                log_message(f"Using GCS URI for large artifact {asset.filename} ({len(artifact_bytes):,} bytes) to prevent history bloat.", Severity.INFO)
                try:
                    ...
                    log_message(f"Failed to save artifact via URI: {e}", Severity.WARNING)
            else:
                log_message(f"Skipping inline save_artifact for {asset.filename} because it is too large ({len(artifact_bytes):,} bytes) and not saved to GCS.", Severity.INFO)
        else:
            try:
                await context.save_artifact(
                    filename=asset.filename,
                ...
                log_message(f"Saved file in artifacts inline. File size: {len(artifact_bytes)}. asset_name: {asset.filename}", Severity.INFO)
            except Exception as e:
                log_message(f"Artifact service unavailable, skipping artifact save: {e}", Severity.WARNING)
...
def store_inline_artifact_metadata(context: CallbackContext, asset: GeneratedMedia, add_to_temp: bool = False):
    ...
    try:
        ...
        if add_to_temp:
            ...
            temp_artifacts = context.state.get(f"{temp_prefix}{TEMP_ARTIFACTS_STATE_KEY}", {})
            if not isinstance(temp_artifacts, dict):
                temp_artifacts = {}
            temp_artifacts[asset.filename] = new_artifact_entry
            context.state[f"{temp_prefix}{TEMP_ARTIFACTS_STATE_KEY}"] = temp_artifacts
