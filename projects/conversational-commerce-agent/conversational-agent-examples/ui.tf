...
resource "google_storage_bucket" "ui_src_bucket" {
  name                        = "${var.project_id}-apparel-search-ui" # Every bucket name must be globally unique
  location                    = "US"
...
resource "google_storage_bucket_object" "ui_static_fiels" {
  for_each = fileset("${path.module}/assets/ui/static", "**/*")
  name     = "static/${each.key}"
  bucket   = google_storage_bucket.ui_src_bucket.name
  source   = "${path.module}/assets/ui/static/${each.key}"
}
...
resource "google_secret_manager_secret_iam_member" "secret-access" {
  ...
  member     = "serviceAccount:${local.target_project_number}-compute@developer.gserviceaccount.com"
